...
def SMA(df: DataFrame, TI: str = "SMA", period: int = [30, 41, 60, 82, 90], column: str = "close"):
    """
...
def SMM(df: DataFrame, TI: str = "SMM", period: int = [9, 18, 27, 45, 81], column: str = "close"):
    """
...
def SSMA(df: DataFrame, TI: str = "SSMA", period: int = [9, 18, 27, 45, 81], column: str = "close", adjust: bool = True):
    """
    ...
    if valid:
        for p in period:
            col_name = "{}{}_{}".format(TI, str(p), column)
            df[col_name] = df[column].ewm(
                ignore_na=False, alpha=1.0 / p, min_periods=0, adjust=adjust).mean()
        return df
        ...
        col_name = "{}{}_{}".format(TI, str(period), column)
        df[col_name] = df[column].ewm(
            ignore_na=False, alpha=1.0 / period, min_periods=0, adjust=adjust).mean()
        return df


def EMA(df: DataFrame, TI: str = "EMA", period: int = [9, 18, 27, 45, 81], column: str = "close", adjust: bool = True):
    """
...
def DEMA(df: DataFrame, TI: str = "DEMA", period: int = [9, 18, 27, 45, 81], column: str = "close", adjust: bool = True):
    """
    ...
    DEMA = 2 * EMA(df, period) - \
        EMA(df, period).ewm(span=period, adjust=adjust).mean()
    valid = False
    # Checks for list of period values
    if len(period) > 1:
        valid = True
    if valid:
        for p in period:
            col_name = "{}{}_{}".format(TI, str(p), column)
            DEMA = 2 * EMA(df, p) - EMA(df, p).ewm(span=p,
                                                   adjust=adjust).mean()
            df[col_name] = DEMA
        return df
...
def TEMA(df: DataFrame, TI: str = "TEMA", period: int = [9, 18, 27, 45, 81], adjust: bool = True):
    """
    ...
    if valid:
        for p in period:
            ...
            TEMA = (
                triple_ema - 3 * EMA(df, p).ewm(span=p,
                                                adjust=adjust).mean() + ema_ema_ema
            )
            #========================================================================#
            df[col_name] = TEMA
        return df
        ...
        ema_ema_ema = (
            EMA(df, period).ewm(ignore_na=False,
                                span=period, adjust=adjust).mean()
            .ewm(ignore_na=False, span=period, adjust=adjust).mean()
        )
        TEMA = (
            triple_ema - 3 * EMA(df, period).ewm(span=period,
                                                 adjust=adjust).mean() + ema_ema_ema
        )
        df[col_name] = TEMA
        return df
...
def TRIMA(df: DataFrame, TI: str = "TRIMA", period: int = [18, 36, 54, 81, 108]):
    """
...
def TRIX(df: DataFrame, TI: str = "TRIX", period: int = [20, 38, 56, 76, 90], column: str = "close", adjust: bool = True):
    """
...
def VAMA(df: DataFrame, TI: str = "VAMA", period: int = [8, 16, 24, 40, 56], column: str = "close"):
    """
...
def ZLEMA(df: DataFrame, TI: str = "ZLEMA", period: int = [26, 12, 9, 36, 48], adjust: bool = True, column: str = "close"):
    """
